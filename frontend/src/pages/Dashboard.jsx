        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <section className="rounded-2xl border border-white/10 bg-slate-900/40 p-6 shadow-xl backdrop-blur-md">
            ...
            <div className="mt-4 grid gap-3 md:hidden">
              ...
              {!(data?.recent_activity && data.recent_activity.length) && <p className="text-sm text-slate-400">No transfers yet.</p>}
            </div>
            <div className="mt-4 hidden overflow-x-auto rounded-xl border border-white/5 bg-slate-950/50 md:block">
              <table className="w-full text-left text-sm text-slate-300">
                ...
                <tbody className="divide-y divide-white/5">
                  ...
                  {!(data?.recent_activity && data.recent_activity.length) && <tr><td className="px-4 py-6 text-center text-slate-400" colSpan="4">No transfers yet.</td></tr>}
                </tbody>
              </table>
            </div>
          </section>
          <section className="rounded-2xl border border-white/10 bg-slate-900/40 p-6 shadow-xl backdrop-blur-md">
            ...
            <div className="mt-4 grid gap-3">
              ...
              {!(data?.recent_alerts && data.recent_alerts.length) && <p className="rounded-xl border border-white/5 bg-white/5 p-4 text-center text-sm text-slate-400">No alerts generated yet.</p>}
            </div>
          </section>
        </div>
        <section className="rounded-2xl border border-white/10 bg-slate-900/40 p-6 shadow-xl backdrop-blur-md">
              ...
              <article key={log?.id || log?.transaction_id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between">
                  <p className="truncate font-mono text-sm font-semibold text-slate-300" title={log?.transaction_id}>
                    {(log?.transaction_id && log.transaction_id.length > 16) ? `${log.transaction_id.substring(0, 16)}...` : log?.transaction_id}
                  </p>
                ...
                {(data?.audit_logs || []).map(log => (
                  <tr key={log?.id || log?.transaction_id} className="transition-colors hover:bg-white/[0.02]">
                    <td className="px-4 py-3 font-mono text-xs text-slate-300" title={log?.transaction_id}>
                      {(log?.transaction_id && log.transaction_id.length > 20) ? `${log.transaction_id.substring(0, 20)}...` : log?.transaction_id}
                    </td>
