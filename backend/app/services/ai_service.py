...
from app.security.privacy.differential_privacy import DifferentialPrivacyService, PrivacyBudgetExhausted
...
class AIService:
    ...
    def analyze_transfer(
        ...
        try:
            # We don't have explicit user_id/transfer_id here, but DP service expects them.
            # Using placeholders since they are primarily for budget tracking per user, but we'll use a global 'system' or fallback to 0.
            dp_result = DifferentialPrivacyService.privatize_features(user_id="system", transfer_id="upload", raw_features=raw_features)
            ...
            raise TransferBlockedError(str(e), anomaly_score=1.0, risk_level="BLOCKED")
        ...
        return {
            ...
            "features": privatized,
            "dp_metadata": {
                "dp_enabled": dp_result.applied,
                "mechanism": dp_result.mechanism,
                "epsilon_used": dp_result.epsilon_spent,
                "remaining_budget": dp_result.remaining_budget,
                "protected_features": dp_result.protected_features
            }
        }